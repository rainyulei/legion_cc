...
async fn test_submit_ticket() {
    ...
    let id = engine.submit_ticket("Fix bug #42".into(), TeamMode::TechLeadTeam, 5).await;
    assert_eq!(id, 1);
...
async fn test_take_next() {
    let engine = OrchestrateEngine::new(2);
    engine.submit_ticket("Task A".into(), TeamMode::Solo, 3).await;
...
async fn test_report_iteration_success() {
    let engine = OrchestrateEngine::new(2);
    engine.submit_ticket("Task B".into(), TeamMode::TechLeadTeam, 5).await;
...
async fn test_report_iteration_retry() {
    let engine = OrchestrateEngine::new(2);
    engine.submit_ticket("Task C".into(), TeamMode::TechLeadTeam, 3).await;
    engine.take_next(1).await;
...
async fn test_report_iteration_max_exceeded() {
    let engine = OrchestrateEngine::new(2);
    engine.submit_ticket("Task D".into(), TeamMode::Solo, 2).await;
    engine.take_next(1).await;
...
async fn test_stop_all() {
    let engine = OrchestrateEngine::new(3);
    engine.submit_ticket("T1".into(), TeamMode::default(), 5).await;
    engine.submit_ticket("T2".into(), TeamMode::default(), 5).await;
    engine.take_next(1).await;
...
async fn test_queue_stats() {
    ...
    engine.submit_ticket("T1".into(), TeamMode::default(), 5).await;
    engine.submit_ticket("T2".into(), TeamMode::default(), 5).await;
    engine.submit_ticket("T3".into(), TeamMode::default(), 5).await;
...
async fn test_worker_idle() {
    ...
    engine.submit_ticket("Task".into(), TeamMode::default(), 5).await;
    engine.take_next(1).await;
    assert!(!engine.is_worker_idle(1).await);
