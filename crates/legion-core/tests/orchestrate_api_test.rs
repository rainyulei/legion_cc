...
async fn test_report_endpoint() {
    ...
    engine.submit_ticket("task-abc".into(), "task-abc".into(), None, None, legion_core::TeamMode::default(), 5, Vec::new(), None, false).await.unwrap();
...
async fn test_stop_all_endpoint() {
    ...
    for i in 1..=3u16 {
        engine.submit_ticket(format!("task-{}", i), format!("task-{}", i), None, None, legion_core::TeamMode::default(), 5, Vec::new(), None, false).await.unwrap();
    }
    for i in 1..=3u16 {
